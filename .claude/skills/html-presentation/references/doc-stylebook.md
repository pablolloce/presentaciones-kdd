...
- Body: Inter 400, 1rem, line-height 1.65, color `--brand-text-sec`
- Eyebrow / labels: JetBrains Mono 600, 0.7rem, uppercase, tracking 0.14em, color `--brand-text-muted`
- Pull quote: Inter 500 italic, 1.25rem, line-height 1.4, color `--brand-text`, left-border 3px NFQ Amber, padding-left 1.25rem

## 3. Color tokens (CSS variables, identical to decks)

```css
:root {
  --brand-bg:           #FAFAF8;   /* warm paper */
  --brand-bg-surface:   #FFFFFF;
  --brand-text:         #0E0E0E;
  --brand-text-sec:     #2C2C2C;
...
[data-theme="dark"] {
  --brand-bg:           #0E0E0E;
  --brand-bg-surface:   #181816;
  --brand-text:         #F5F5F0;
  --brand-text-sec:     #C7C7C2;
  --brand-text-muted:   #8A8A85;
  --brand-rule:         #2A2A28;
}
[data-theme="glass"] {
  --brand-bg:           rgba(250,250,248,0.62);
  --brand-bg-surface:   rgba(255,255,255,0.55);
...
- `.formula` — KaTeX block with `--brand-rule` border-top + bottom (concept only)
...
Identical pattern to decks: three buttons (Light / Dark / Glass) in the masthead top-right, set `data-theme` on `<html>`, persist in `localStorage` under key `deck-doc-theme`. Glass theme requires a soft photographic gradient backdrop on `body::before`.
