---
name: design-polish
description: Final quality pass for a presentation in this repository before client delivery. Fixes alignment, spacing, consistency, interaction states, and detail issues. Use after the interface is functionally complete, before shipping presentations, proposals, or dashboards.
user-invocable: true
---

Perform a meticulous final pass on a presentation in `presentations/`. Fix every detail that separates "done" from "polished." This is the last step before client delivery.

## Pre-Polish Check

1. **Is it functionally complete?** Polish is the last step, not the first. Don't polish incomplete work.
2. **Carga el contexto de diseño**: `CLAUDE.md` (§ 3-4), el estilobook correspondiente en
   `.claude/skills/html-presentation/references/` y `brand/brand.json` (paleta activa).
...
- **Contrasta con los estándares**: ¿cumple CLAUDE.md § 3-4 y el estilobook del formato?
