...
The template exposes `window.NFQ3D` with a `registry` mapping scene kinds to builder functions and a `scenes` object listing currently mounted instances. Mount any scene by adding a div with `data-nfq-scene="<kind>"` to a slide. The bootstrap function picks the matching builder at load and constructs the DOM tree in place.

```html
<div id="nfq-hero-3d"      data-nfq-scene="hero"          style="position:absolute;inset:0"></div>
<div id="nfq-divider-3d-1" data-nfq-scene="particles"     style="position:absolute;inset:0;opacity:0.55;pointer-events:none;z-index:0"></div>
<div                       data-nfq-scene="tunnel"        style="position:absolute;inset:0;opacity:0.7"></div>
<div                       data-nfq-scene="energy"        style="position:absolute;inset:0;opacity:0.85"></div>
<div                       data-nfq-scene="riskSurface"   style="position:absolute;inset:0"></div>
<div                       data-nfq-scene="balanceFold"   style="position:absolute;inset:0"></div>
<div                       data-nfq-scene="flowGraph"     style="position:absolute;inset:0"></div>
<div                       data-nfq-scene="dataMonolith"  style="position:absolute;inset:0"></div>
```

Unknown `data-nfq-scene` values log a warning but do not break the page. WebGL availability is irrelevant — the scenes are pure CSS/SVG.

**Scene continuity** is wired via CSS: `[data-nfq-scene]` has a `.6s` opacity transition, and `.slide:not(.active) [data-nfq-scene] { opacity: 0 }` fades scenes in and out as slides change.
