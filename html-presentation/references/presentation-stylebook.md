...
### Mandatory elements
- **NFQ Logo**: The NFQ isotipo (color SVG) MUST appear on every slide. Use the inline SVG `<symbol>` pattern with `<use href="#nfq-iso"/>`. On the cover: 28x35px in the header. On all other slides: 20x25px fixed top-left (as a persistent UI element alongside the section tag). The logo file is at `references/nfq-isotipo.svg`.
