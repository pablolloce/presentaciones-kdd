...
description: Create premium single-file HTML presentations and web-based artifacts for nfq advisory. Use when the user asks for an HTML presentation, interactive deck, premium scroll document, web-based client deliverable, or when another skill needs a polished HTML artifact. This is the shared implementation layer for advisory HTML outputs; pair it with `frontend-design` for creative direction.
---

Build premium HTML slide decks AND long-scroll documents that open directly in the browser. McKinsey-grade consulting aesthetic, NFQ visual system (Inter + JetBrains Mono, dark-first, NFQ Amber/Coral/Violet).
...
3. Read `references/nfq-isotipo.svg` — the NFQ logo SVG that must appear on every slide.
...
- **NFQ Logo**: Inline SVG `<symbol>` with `<use href="#nfq-iso"/>`. Fixed 20x25px top-left on all slides. Larger (28x35px) on cover.
...
### NFQ brand colors (for visualizations)
```
Amber:  #F48B4A    Coral:  #E04870    Violet: #9B59B6
Steel:  #5B7FC7    Azure:  #3B82F6
```
Use ONLY these for chart colors, category markers, and icon backgrounds. Never generic Tailwind colors.
...
- **Pre-captured exports (MANDATORY when the deck contains embedded images)**: After creating any deck, run `capture-slides.mjs` (at `references/capture-slides.mjs`) to embed pixel-perfect slide images as `_PRECAPTURED`. PDF/IMG/PPTX buttons then bypass html2canvas entirely. Usage:
  ```
  node capture-slides.mjs <presentation.html> [--theme light|dark|glass] [--lang es|en] [--width 1280 --height 720]
  ```
  ...
  **Defaults:** `--width 1280 --height 720` (the script's argv parser fails if these are omitted because of an `indexOf` quirk — always pass them). Run once per active language to populate `_PRECAPTURED` (ES) and `_PRECAPTURED_EN` (EN); `getPrecaptured()` auto-selects based on `currentLang`. Re-run after any slide edit, otherwise the array drifts from the source HTML and the export shows stale content.
...
3. Read `references/nfq-isotipo.svg` — the NFQ logo SVG.
...
- **Print CSS**: every doc has the `@media print` block from doc-stylebook § 7. PDF export = browser print (Cmd+P), NOT `capture-slides.mjs` (which is deck-only).
...
- Use Tailwind Play CDN (`<script src="https://cdn.tailwindcss.com">`).
...
- **Export-safe (default):** `references/threejs-scenes.md`. Pinned `three@0.184.0`, dispose pattern, color management, fat-line diagrams. Captures cleanly with `capture-slides.mjs`. Use with `presentation-template.html`. Predefined scenes: Architecture Layers, Particle Network, Rotating Solid, Fat-Line Flow.
...
- `references/capture-slides.mjs` — Playwright pre-capture helper for decks. Auto-detects template mode and adjusts settle timing. **Deck-only** — docs use browser print for PDF.
- `references/capture-thumb.mjs` — single-PNG thumbnail capture helper (gallery thumbnails). Does not modify source HTML.
